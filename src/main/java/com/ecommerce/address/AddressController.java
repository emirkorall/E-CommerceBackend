...
public class AddressController {
    ...
    public ResponseEntity<AddressResponse> getAddressById(@PathVariable long id, @AuthenticationPrincipal User user) {
        return ResponseEntity.ok(addressService.findAddressById(id, user));
    }

    @PostMapping

    public ResponseEntity<AddressResponse> createAddress(@RequestBody AddressRequest request, @AuthenticationPrincipal User user) {
        return ResponseEntity.status(201).body(addressService.saveAddress(request, user));
    }

    @PutMapping("/{id}")

    public ResponseEntity<AddressResponse> updateAddress(@PathVariable long id, @RequestBody AddressRequest request, @AuthenticationPrincipal User user) {
        return ResponseEntity.ok(addressService.updateAddress(id, request, user));
    }

    @DeleteMapping("/{id}")

    public ResponseEntity<AddressResponse> deleteAddress(@PathVariable long id, @AuthenticationPrincipal User user) {
